...
import { GeneralColorToArray } from '../helper/utils';
  ...
  return (
    ...
    <div className='min-h-[50rem] bg-blue-300 p-8 grid grid-cols-2 relative -z-[0] utsm:flex utsm:flex-col'>
        ...
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Brand: &nbsp;
            <input className='text-2xl text-stone-700 h-10 font-oswald' value={brand} onChange={e=>setBrand(e.target.value)}/>
            <button onClick={()=>{}} className='h-10 bg-stone-800 w-8'>+</button>
        </label>
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Name: &nbsp;
            <input className='text-2xl text-stone-700 h-10 font-oswald' value={name} onChange={e=>setName(e.target.value)}/>
            <button onClick={()=>{}} className='h-10 bg-stone-800 w-8'>+</button>
        </label>
        ...
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Color: &nbsp;
            <input className='text-2xl text-stone-700 h-10 rounded-l-sm font-oswald' value={color} onChange={e=>setColor(e.target.value)}/>
            <button onClick={()=>{}} className='h-10 bg-stone-800 w-8 rounded-r-sm'>+</button>
        </label>
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            General Color: &nbsp;
        ...
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Description: &nbsp;
            <input className='text-2xl text-stone-700 h-10 rounded-l-sm font-oswald' value={desc} onChange={e=>setDesc(e.target.value)}/>
            <button onClick={()=>{}} className='h-10 bg-stone-800 w-8 rounded-r-sm'>+</button>
        </label>
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Price: &nbsp;
            <input className='text-2xl text-stone-700 h-10 rounded-l-sm font-oswald' value={price} onChange={e=>setPrice(prev => {
        ...
        <label htmlFor="" className='flex p-1 items-center font-bold bg-blue-500'>
            Image: &nbsp;
            <input className='text-2xl text-stone-700 h-10 font-oswald' value={link} onChange={e=>setLink(e.target.value)}/>
            <button onClick={()=>{setImages(prev => [...prev,link]);setLink("")}} className='h-10 bg-stone-800 w-8'>+</button>
        </label>
...
const ColorDropdown = (props:any) => {
  ...
  return (
    <div>
        <button onClick={()=>{setOpen(prev => !prev)}} className='h-10 w-36 rounded-l-sm bg-white px-2 overflow-hidden inline-block text-ellipsis whitespace-nowrap' >{color == null ? `Color ►` : GENERALCOLOR[colors.indexOf(color)]}</button>
